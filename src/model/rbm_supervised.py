...
class RBM(object):
    ...
    def get_cost_updates(self, lr=0.1, persistent=None, k=1):
        ...
        pre_sigmoid_rv, rv_mean = self.propdown(ph_mean)
        ...
        if not self.topic_input:
            ...
            # detaW = T.dot(self.input.transpose(),ph_mean) - T.dot(chain_end.transpose(),nh_means[-1])
            # detaV = self.input - chain_end
            # detaH = ph_mean - nh_means[-1]
            ...
            # hparam = T.grad(cost, self.hbias)
            # vparam = T.grad(cost, self.vbias)
            # updates[self.W] = self.W - wparam * T.cast(lr,dtype=theano.config.floatX)
            # updates[self.hbias] = self.hbias - hparam * T.cast(lr,dtype=theano.config.floatX)
            # updates[self.vbias] = self.vbias - vparam * T.cast(lr,dtype=theano.config.floatX)
            #     # constructs the update dictionary
            # detaW = T.dot(self.input.transpose(),ph_mean) - T.dot(chain_end.transpose(),nh_means[-1])
            # detaV = self.input - chain_end
            # detaH = ph_mean - nh_means[-1]
            # # detaTW = T.dot(self.topic_input.transpose(),th_mean) - T.dot(chain_end.transpose(),nh_means[-1])
            # # detaTV = self.topic_input - chain_end
            # # detaTH = th_mean - nh_means[-1]
            # updates[self.W] = self.W + (detaW)* T.cast(lr,dtype=theano.config.floatX)
            # updates[self.vbias] = self.vbias + T.mean((detaV), axis=0)* T.cast(lr,dtype=theano.config.floatX)
            # updates[self.hbias] = self.hbias + T.mean((detaH ), axis=0)* T.cast(lr,dtype=theano.config.floatX)
