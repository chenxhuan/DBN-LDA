...
class dA(object):
    ...
    def get_cost_updates(self, corruption_level, learning_rate, type=None, lamda =0.01):
        ...
        if type==None:
            cost = T.mean(L)
        if type==1:
            tilde_topic = self.get_corrupted_input(self.topic_input, corruption_level)
            topic_y = self.get_hidden_values(tilde_topic)
            TL = - T.sum(topic_y * T.log(y) + (1 - topic_y) * T.log(1 - y), axis=1)
            cost = T.mean(L) + T.mean(TL)*T.cast(lamda,dtype=theano.config.floatX)
        if type==2:
            cost = T.mean(L) + self.labelSupervised_cost*T.cast(lamda,dtype=theano.config.floatX)
