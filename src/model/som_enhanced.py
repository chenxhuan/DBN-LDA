...
class SOM:
    ...
    def train(self, iterations=1000, train_vector_x=[[0.0]], train_vector_y=[0.0],batch_update=True):
        ...
        for i in range(1,iterations+1):
            if batch_update:
                ...
                shuffle(train_vector_x) # Randomize the training data, so it gets presented in a different order each time
            ...
            for  j in range(len(train_vector_x)):
                if not batch_update:
                    FV_delta = [[0.0]*self.FV_size]*self.total
                    PV_delta = [[0.0]*self.PV_size]*self.total
                
                input_FV=train_vector_x[j]
                input_PV=train_vector_y[j]
                ...
                for k in range(self.total):
                    ...
                    if dist < radius_decaying:
                        ...
                        PV_delta[k] = [PV_delta[k][l] + inf_lrd*(input_PV-self.nodes[k].PV[l]) for l in range(self.PV_size)]
    ...
    def getResult(self,test=[0.0]):
        results = []
        for v in test:
            c = self.predict(v)[0]
            results.append(int(round(c)))
        return results
if __name__ == "__main__":
    print "Initialization..."
    a=SOM(100,100,2,1,False,0.05)

    print "Training for the XOR function -- using Batch Update..."
    a.train(100,[[1,0],[1,1],[0,1],[0,0]],[1,0,1,0])
